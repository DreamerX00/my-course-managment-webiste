...
import { generateJSON as generateJSONFromHTML } from '@tiptap/html';
...
/**
 * Imports a file (MD, TXT, DOCX) and converts it to Tiptap JSON format
...
export async function importFileToTiptapJSON(file: File): Promise<Record<string, unknown>> {
  ...
  if (ext === 'docx' || ext === 'doc') {
    ...
    return generateJSONFromHTML(html, [StarterKit]);
  } else if (ext === 'md' || ext === 'markdown' || ext === 'txt') {
    const text = await file.text();
    return generateJSON(text, [StarterKit]);
  }
  
  throw new Error('Unsupported file type. Please upload a .docx, .doc, .md, or .txt file.');
}
